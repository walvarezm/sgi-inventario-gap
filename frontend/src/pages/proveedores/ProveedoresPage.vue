  ...
  <q-page class="sgi-page">
    ...
    <div class="row items-center q-mb-lg">
      <div>
        ...
        <div class="text-muted text-body2 q-mt-xs">Gestión de proveedores y órdenes de compra</div>
      </div>
      <q-space />
            ...
            <template #body-cell-accionesOrden="{ row }">
              <q-td class="text-right">
                ...
                <q-btn
                  ...
                  color="negative"
                  size="sm"
                  @click="confirmarCancelacion(row)"
                >
...
const columnasOrden: QTableColumn[] = [
  ...
  },
  {
    ...
    field: 'sucursalId',
    align: 'left',
    format: (v) => sucursalStore.getById(v)?.nombre ?? v,
  },
  {
    name: 'fechaEmision',
    label: 'Fecha',
...
function colorEstado(estado: EstadoOrden): string {
  return ESTADO_ORDEN_COLOR[estado] ?? 'grey'
}
