          ...
          >
            <template #body-cell-tipo="{ value }">
              <q-td>
                <q-chip
                  dense
                  size="sm"
                  :color="colorMovimiento(value)"
                  text-color="white"
                  :icon="iconoMovimiento(value)"
                  :label="value"
                />
              </q-td>
            </template>
            ...
            <template #body-cell-productoNombre="{ row }">
              <q-td>
                ...
                <div class="text-caption text-muted">
                  [{{ row.productoMarca }}] {{ row.productoSku }}
                </div>
              </q-td>
            </template>
            ...
            <template #body-cell-fecha="{ row }">
              <q-td>
                ...
                <div class="text-caption text-muted">{{ formatDate(row.fechaRegistro) }}</div>
                <!--                <div class="text-caption text-muted">{{ formatTime(row.fecha) }}</div>-->
              </q-td>
            </template>
            <template #body-cell-notas="{ row }">
              <q-td>
                <div>{{ row.notas }}</div>
                <div class="text-caption text-muted">{{ row.detalleAccion }}</div>
              </q-td>
            </template>
...
async function cargarReportes(): Promise<void> {
  cargando.value = true
  //await Promise.all([cargarVentas(), cargarTop(), cargarStock(), cargarMovimientos()])
...
async function cargarMovimientos(): Promise<void> {
  ...
  try {
    ...
    movimientos.value = movimientos.value.sort((a, b) => {
      // 1. Ordenar por MARCA (A-Z)
      const marcaCompare = a.productoMarca.localeCompare(b.productoMarca)
      if (marcaCompare !== 0) return marcaCompare
      // 2. Ordenar por SKU (A-Z)
      const skuCompare = a.productoSku.localeCompare(b.productoSku)
